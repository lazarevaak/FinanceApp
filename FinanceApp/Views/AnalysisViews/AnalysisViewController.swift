...
final class AnalysisViewController: UIViewController {
    var onBack: (() -> Void)?
        ...
        init(direction: Direction, accountId: Int) {
            self.viewModel = AnalysisViewModel(direction: direction, accountId: accountId)
    ...
    private func setupCustomHeader() {
        ...
        backButton.setTitleColor(UIColor(named: "IconColor"), for: .normal)
    ...
    private func separatorView(leftInset: CGFloat = 16) -> UIView {
        let container = UIView()
        container.translatesAutoresizingMaskIntoConstraints = false

        let line = UIView()
        line.backgroundColor = UIColor.systemGray4.withAlphaComponent(0.6)
        ...
        NSLayoutConstraint.activate([
            ...
            line.leadingAnchor.constraint(equalTo: container.leadingAnchor, constant: leftInset),
    ...
    @objc private func sortOptionChanged(_ sender: UISegmentedControl) {
        let selected = sender.titleForSegment(at: sender.selectedSegmentIndex) ?? "По дате"
        viewModel.sortTitle = selected
    }
